...
public:
    ...
    int16_t getWaveValue()
    {
        ...
        {
        case Wave::SQU:
            _value = index < _widthHalf ? _heightM1 : 0;
            break;
        case Wave::SAW:
            _value = indexHeight;
            break;
        case Wave::TRI:
            _value = index < _widthHalf ? (indexHeight << 1) : ((_heightM1 - indexHeight) << 1);
            break;
        case Wave::NOISE:
            _value = _rnd.getRandom16(_height);
            break;
        default:
            _value = 0;
            break;
        ...
        if (_signedOut)
        {
            _value = _value - (_height >> 1);
        }

        return _value >> _attenuate;
    }
